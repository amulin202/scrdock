...
- **设备页**：1.5 秒轮询 `adb devices`（仅增量更新变化的行和字段，保留选择、滚动位置与输入焦点，插拔/授权约 1~2 秒内反映），
  列出状态（在线/离线/待授权）、序列号、型号、Android 版本、连接类型、电量、
  **投屏状态**（●当前 = 工具栏正在控制的一路，● = 后台会话）；
  选中一行 → **连接所选**（双击同效）：已有会话则切换为活动会话（不重启），
  否则新开一路；**断开投屏**关闭选中行（或当前）的会话；**断开无线**关闭所选
  无线设备的投屏并取消该会话重连，然后断开该设备的 ADB 网络连接（不影响其他设备，
  不关闭手机上的无线调试服务，也不删除手机的配对授权）；勾选
  **记住此设备** 则把 `serial=` 写进 ini（不勾 = 自动模式：跟随唯一在线设备）。
- **无线行**：`无线(--tcpip)` 开关（经 USB 一键切 WiFi）、无线地址手动
  `adb connect`（IP 地址和端口分开输入，端口默认 **5555**，通常只需填写手机 IP）、
  **配对…** 打开 Android 11+ 无线配对窗（默认支持**生成二维码手机扫码自动配对**与连接，亦可切换为手动输入 ip:端口 + 配对码）。
  等待扫码不再于 60 秒后失效；兼容小米 Android 15 带引号的服务名及配对口令，
  手机请使用“开发者选项 → 无线调试 → 使用二维码配对设备”，不要使用普通相机扫码。
...
二维码测试覆盖服务发现、配对、连接和延迟扫码；安装测试依赖后还会独立解码生产绘制的二维码：
`python -m pip install --target build\qr-test-deps zxing-cpp Pillow`。
可编辑输入框与按钮等高，文本水平、垂直居中；附加参数使用单行输入框。
